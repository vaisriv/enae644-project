...
# Training configuration (used by: uv run adversarial-planning-train)
training:
    observer:
        hidden_dim: 64
        num_epochs: 100
...
│   └── ...
└── models/                       # Generated by: uv run adversarial-planning-train
...
# Save model (done by: uv run adversarial-planning-train)
observer_net = TrajectoryClassifier(...)
eqx.tree_serialise_leaves("outputs/models/observer_rnn.eqx", observer_net)

# Load model (done by: uv run adversarial-planning)
loaded_observer = eqx.tree_deserialise_leaves("outputs/models/observer_rnn.eqx", observer_net)
```
...
```python
# 1. Train models (run once, writes checkpoints to outputs/models/)
