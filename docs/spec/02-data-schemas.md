...
│   └── ...
└── checkpoints/  (optional, for saving intermediate states)
    └── ...

models/
├── observer_rnn.eqx              # Trained RNN observer
├── irl_reward.eqx                # Trained IRL reward function
└── ...
...
eqx.tree_serialise_leaves("models/observer_rnn.eqx", observer_net)

# Load model
loaded_observer = eqx.tree_deserialise_leaves("models/observer_rnn.eqx", observer_net)
```
...
observer_net = load_observer("models/observer_rnn.eqx")
irl_model = load_irl_model("models/irl_reward.eqx")

# 3. Run simulation
result = run_simulation(config, observer_net, irl_model, key=jax.random.PRNGKey(42))
