...
def sparse_obstacle_workspace() -> Workspace:
    ...
    obstacles = [
        ...
        PolygonObstacle(vertices=jnp.array([[5.0, 1.0], [6.0, 1.0], [5.5, 2.0]])),
    ]
...
def dense_obstacle_workspace() -> Workspace:
    ...
    for x in range(2, 9, 2):
        for y in range(2, 9, 2):
            obstacles.append(
                CircleObstacle(center=jnp.array([float(x), float(y)]), radius=0.4)
            )

    # Add some polygon obstacles
    obstacles.append(
        PolygonObstacle(
            vertices=jnp.array([[0.5, 4.5], [1.5, 4.5], [1.5, 5.5], [0.5, 5.5]])
        )
    )

    obstacles.append(
        PolygonObstacle(
            vertices=jnp.array([[8.5, 4.5], [9.5, 4.5], [9.5, 5.5], [8.5, 5.5]])
        )
    )
...
def corridor_workspace() -> Workspace:
    ...
    obstacles = [
        # Top wall obstacles
        PolygonObstacle(
            vertices=jnp.array([[0.0, 3.5], [8.0, 3.5], [8.0, 5.0], [0.0, 5.0]])
        ),
        PolygonObstacle(
            vertices=jnp.array([[12.0, 3.5], [20.0, 3.5], [20.0, 5.0], [12.0, 5.0]])
        ),
        # Bottom wall obstacles
        PolygonObstacle(
            vertices=jnp.array([[0.0, 0.0], [8.0, 0.0], [8.0, 1.5], [0.0, 1.5]])
        ),
        PolygonObstacle(
            vertices=jnp.array([[12.0, 0.0], [20.0, 0.0], [20.0, 1.5], [12.0, 1.5]])
        ),
    ]
    return Workspace(bounds=bounds, obstacles=obstacles)
...
def single_circle_workspace() -> Workspace:
    ...
    bounds = jnp.array([[0.0, 10.0], [0.0, 10.0]])
    obstacle = CircleObstacle(center=jnp.array([5.0, 5.0]), radius=1.5)
