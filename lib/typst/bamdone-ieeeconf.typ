...
#let ieee(
    ...
    set page(
        ...
        numbering: if draft {
            (..it) => strong(text(
                red,
                [DRAFT, #numbering("1 / 1", it.at(0)) of #numbering("1 / 1", it.at(1))],
            ))
        } else { none },
        header: if draft {
    ...
    show heading: it => {
        ...
        if it.level == 1 {
            ...
            // Third level headings are run-ins too, but start the paragraph
            // immediately (instead of on a newline).
            // Spec: run-in, italic, sentence case, indented like a regular paragraph
            // (inherits the 1em first-line-indent), followed by a colon and
            ...
            // Fourth level headings are the same as third level, but alphas
            // not nums and are indented further.
            // Spec: run-in, italic, sentence case, indented slightly further
            // than a regular paragraph. (The extra h(1em) stacks on top of the
            // paragraph's own first-line-indent.)
            set text(style: "italic")
            h(1em)
            if it.numbering != none {
                numbering("a)", deepest)
    ...
    let author-name-affil = authors
        .map(
            author => [#author.given #author.surname#super[#author.affiliation]],
        )
        .join(", ", last: ", and ")
        ...
        align(left)[
            #block(width: 100%, height: auto, stroke: none)[#text(8.5pt)[
                ...
                #for i in range(affiliations.len()) {
                    ...
                    let authors-for-this-affiliation = authors.filter(
                        author => (
